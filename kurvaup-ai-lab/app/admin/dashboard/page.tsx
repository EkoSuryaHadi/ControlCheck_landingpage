...
export default async function AdminDashboard() {
  ...
  const eventResult = db ? await db.from('analytics_events').select('event_name,product_slug,visitor_id,session_id,created_at').gte('created_at', new Date(Date.now()-30*86400000).toISOString()).limit(10000) : { data: [] as any[] }
  const feedbackResult = db ? await db.from('feedback').select('*',{count:'exact',head:true}) : { count: 0 }

  // Supabase returns `data: null` when a query fails, so normalize it before analytics calculations.
