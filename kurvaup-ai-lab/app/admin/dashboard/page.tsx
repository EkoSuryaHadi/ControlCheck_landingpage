...
export default async function AdminDashboard() {
  ...
  const eventResult = db
    ? await db
        ...
        .select(
          'event_name,product_slug,visitor_id,session_id,created_at'
        )
        .gte(
          'created_at',
          since30.toISOString()
  ...
  const feedbackResult = db
    ? await db
        .from('feedback')
        .select(
          'id,product_slug,name,rating,message,created_at'
        )
        .order('created_at', {
          ascending: false,
        })
  ...
  const returningUsers =
    Array.from(
      sessionsByVisitor.values()
    ).filter(
      (set) => set.size > 1
    ).length

  const retentionRate = pct(
    returningUsers,
    Math.max(
      1,
      sessionsByVisitor.size
    )
  )
  ...
  const ranking = products
    .map((product) => {
      ...
      const views = pe.filter(
        (e) =>
          e.event_name ===
          'product_view'
      ).length

      const tries = pe.filter(
        (e) =>
          e.event_name ===
          'try_free_click'
      ).length
      ...
      const productSessionsByVisitor =
        new Map<
          string,
          Set<string>
        >()
      ...
      pe.forEach((e) => {
        ...
        if (
          !productSessionsByVisitor.has(
            e.visitor_id
          )
        ) {
          productSessionsByVisitor.set(
            e.visitor_id,
            new Set()
          )
        }

        productSessionsByVisitor
          .get(e.visitor_id)
          ?.add(e.session_id)
      })
      ...
      const returning =
        Array.from(
          productSessionsByVisitor.values()
        ).filter(
          (set) => set.size > 1
        ).length

      const productRetention = pct(
        returning,
        Math.max(
          1,
          productSessionsByVisitor.size
        )
      )
      ...
      const current = pe.filter(
        (e) =>
          new Date(e.created_at) >=
          since7
      ).length

      const previous = pe.filter(
        (e) => {
          const date = new Date(
            e.created_at
          )
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <span className="h-2 w-2 rounded-full bg-glow shadow-[0_0_18px_rgba(57,214,200,.85)]" />

            <p className="text-xs font-semibold tracking-[.24em] text-glow">
              ADMIN ANALYTICS · V2
            </p>
          </div>
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-[1.55fr_.85fr]">
        <div className="card p-5 lg:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="rounded-xl border border-white/10 bg-white/[.03] px-3 py-2 text-right">
            ...
            {funnel.map(
              (step, index) => (
                <div key={step.label}>
                  ...
                  <div className="h-9 overflow-hidden rounded-xl bg-white/[.045]">
                    <div
                      className="flex h-full items-center rounded-xl border border-glow/10 bg-glow/15 px-3 text-xs font-medium text-glow"
                      style={{
                        width: `${Math.max(
                          step.value
                            ? 18
      ...
      <section className="card mt-6 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-left text-sm">
            <thead className="bg-white/[.015] text-[11px] uppercase tracking-wider text-white/35">
              <tr>
                <th className="p-4">
                  #
                </th>
            ...
            <tbody>
              {ranking.map(
                (r, index) => (
                  <tr
                    className="border-t border-white/5 transition hover:bg-white/[.02]"
                    key={
                      r.product.slug
                    }
                  >
                    <td className="p-4 text-white/25">
                      {String(
                        index + 1
                      ).padStart(
                        2,
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <span className="min-w-14 font-semibold text-glow">
                          {r.pvs}/100
                        </span>
                        ...
                        className={`inline-flex rounded-full border px-2.5 py-1 text-xs ${
                          ...
                          'positive'
                            ...
                            : r
                                ...
                                .tone ===
                              'neutral'
                            ? 'border-blue-300/15 bg-blue-300/5 text-blue-200'
